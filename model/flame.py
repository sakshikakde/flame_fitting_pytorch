...
class FLAME(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Mediapipe embedding for flame
        if config.use_mediapipe_embedding  and hasattr(config, "mediapipe_embedding_path"):
            mediapip_lmk = np.load(config.mediapipe_embedding_path, allow_pickle=True)
            mediapipe_lmk_face_idx = mediapip_lmk["lmk_face_idx"].astype(np.int64)
            mediapip_lmk_bary_coords = mediapip_lmk["lmk_b_coords"]
            mediapipe_lmk_idx = mediapip_lmk["landmark_indices"].astype(np.int64)
            self.register_buffer(
                "lmk_faces_idx",
                torch.tensor(mediapipe_lmk_face_idx, dtype=torch.long),
            )
            self.register_buffer(
                "lmk_bary_coords",
                torch.tensor(mediapip_lmk_bary_coords, dtype=self.dtype),
            )
            self.register_buffer(
                "lmk_idx",
                torch.tensor(mediapipe_lmk_idx, dtype=torch.int16),
            )
        else:
            # Static and Dynamic Landmark embeddings for FLAME
            if hasattr(config, "static_landmark_embedding_path"):
            ...
            if self.use_face_contour and hasattr(
